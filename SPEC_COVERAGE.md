...
- **Google My Business live review sync** — `POST /api/gmb/sync` (+ `/sync/:profileId`, `GET /sync/status`) pulls reviews from the Google Business Profile API into `gmb_reviews` and stamps `last_synced_at`. No-ops with `{ configured: false }` until `GOOGLE_GMB_ACCESS_TOKEN` (or `GOOGLE_GMB_CLIENT_ID/_SECRET/_REFRESH_TOKEN`) are set. Local GMB CRUD is unaffected. Tenant-scoped + covered by `gmb-sync.test.ts`.
